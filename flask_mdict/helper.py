...
def init_mdict(mdict_dir, index_dir=None):
    ...
    dictdirs = os.listdir(mdict_dir)
    for dictdir in sorted(dictdirs):
        for root, dirs, files in os.walk(os.path.join(mdict_dir, dictdir), followlinks=True):
            logger.info("dirs %s files %s" % (str(sorted(dirs)), str(sorted(files))))
            for fname in files:
